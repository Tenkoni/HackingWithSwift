...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate  {
    ...
    override func viewDidLoad() {
        ...
        if let savedPeople = defaults.object(forKey: "people") as? Data {
            if let decoded = try? NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(savedPeople) as? [Person] {
                people = decoded
            }
        }
    }
    ...
    override func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        ...
        let ac2 = UIAlertController(title: "What do you want to do?", message: nil, preferredStyle: .alert)
        ac2.addAction(UIAlertAction(title: "Rename", style: .default){[weak self] _ in
            self?.renamePicture(person)
        })
    ...
    func renamePicture(_ person: Person) {
